...
with open("data.json", encoding='UTF-8') as f:
    ...
    # 取得 skill 的值
    print("firmwareVersion =", p["firmwareVersion"])
    v1 = p["firmwareVersion"]
...
def doLoader_esp_write_flash(port):
    ...
    if(Mreturn == 0):
        ...
        messagebox.showinfo("主机板烧录 工厂专用", "ESP烧入错误 \n 请重新开启程式再继续")
...
def doLoader_nxp_1():
    ...
    response_ = os.system(str(os.getcwd())+"/nxp_bin/sdphost.exe  -t 50000 -p " + \
                                  port+" -- write-file 539001344 "+str(os.getcwd())+"/nxp_bin/ivt_flashloader.bin")
    if response_ == 0:
        print("前置動作_NXP_1成功")
        intProgress = 60
        # updateUI_Loader_writing()
        os.system(str(os.getcwd(
        ))+"/nxp_bin/sdphost.exe  -t 50000 -p "+port+" -- jump-address 539001344")
        time.sleep(0.1)
        doLoader_nxp_2(port)
    else:
        print("response_="+str(response_))
        pop_up_failure("NXP烧入错误")
        updateUI_Loader_finish()
# 2.前置動作_NXP_2
def doLoader_nxp_2(port):
    ...
    response_ = os.system(
        str(os.getcwd())+"/nxp_bin/blhost.exe -p "+port+" -- get-property 1 0")
    if response_ == 0:
        print("前置動作_NXP_2成功")
        intProgress = 65
        # updateUI_Loader_writing()
        time.sleep(0.1)
        doLoader_nxp_2_1(port)
    else:
        print("response_="+str(response_))
        pop_up_failure("NXP烧入错误")
        updateUI_Loader_finish()
# 2.前置動作_NXP_2_1
def doLoader_nxp_2_1(port):
    global intProgress
    print("前置動作_NXP_2_1...port ="+port)
    response_ = os.system(
        str(os.getcwd())+"/nxp_bin/blhost.exe -p "+port+" -- get-property 24 0")
    if response_ == 0:
        print("前置動作_NXP_2_1成功")
        intProgress = 65
        # updateUI_Loader_writing()
        time.sleep(0.1)
...
def doLoader_nxp_3_3(port):
    ...
    response_ = os.system(str(os.getcwd(
    ))+"/nxp_bin/blhost.exe -p "+port+" -- efuse-read-once 0x7")
    if response_ == 0:
        print("前置動作_NXP_3_3成功")
        intProgress = 75
        # updateUI_Loader_writing()
        time.sleep(0.1)
        doLoader_nxp_3_4(port)
    else:
        print("response_="+str(response_))
        pop_up_failure("NXP烧入错误")
        updateUI_Loader_finish()
# 5.晶片初始檢查_NXP_3_4
def doLoader_nxp_3_4(port):
    global intProgress
    print("晶片初始檢查_NXP_3_4...port ="+port)
    response_ = os.system(str(os.getcwd(
    ))+"/nxp_bin/blhost.exe -p "+port+" -- efuse-read-once 0x6")
    if response_ == 0:
        print("前置動作_NXP_3_4成功")
        intProgress = 75
        # updateUI_Loader_writing()
        time.sleep(0.1)
...
def doLoader_nxp_5_1(port):
    ...
    response_ = os.system(str(os.getcwd(
    ))+"/nxp_bin/blhost.exe -p "+port+" -j -- fill-memory 538976256 4 3221225479 word")
    if response_ == 0:
...
def doLoader_nxp_5_2(port):
    ...
    response_ = os.system(str(os.getcwd(
    ))+"/nxp_bin/blhost.exe -p "+port+" -j -- fill-memory 538976260 4 0 word")
    if response_ == 0:
        print("配置FLASH_NXP_5_2 成功")
        intProgress = 88
        # updateUI_Loader_writing()
        time.sleep(0.1)
        doLoader_nxp_5_3(port)
    else:
        print("response_="+str(response_))
        pop_up_failure("NXP烧入错误")
        updateUI_Loader_finish()
# 9.配置FLASH_NXP_5_3
def doLoader_nxp_5_3(port):
    global intProgress
    print("配置FLASH_NXP_5_3...port ="+port)
    response_ = os.system(str(os.getcwd(
    ))+"/nxp_bin/blhost.exe -p "+port+" -j  -- configure-memory 9 538976256")
    if response_ == 0:
        print("配置FLASH_NXP_5_3 成功")
        intProgress = 88
        # updateUI_Loader_writing()
        time.sleep(0.1)
        doLoader_nxp_6(port)
...
def doLoader_nxp_6(port):
    ...
    response_ = os.system(str(os.getcwd(
    ))+"/nxp_bin/blhost.exe -p "+port+" -j -- flash-erase-region 1610612736 110592 9")
    if response_ == 0:
...
def doLoader_nxp_8(port):
    ...
    response_ = os.system(str(os.getcwd())+"/nxp_bin/blhost.exe -p "+port + \
                                  " -j -- write-memory 1610612736 "+str(os.getcwd())+"/nxp_bin/main_nxp.bin 9")
    if response_ == 0:
...
def doLoader_nxp_9(port):
    ...
    response_ = os.system(str(os.getcwd())+"/nxp_bin/blhost.exe -p "+port + \
                                  " -j -- read-memory 1610612736 8192 "+str(os.getcwd())+"/nxp_bin/flexspi.dat")
    if response_ == 0:
...
def pop_up_success():
    ...
    messagebox.showinfo("主机板烧录 工厂专用", "烧录成功")
...
def pop_up_failure(strMsg):
    global intProgress
    intProgress = 404
    messagebox.showinfo("主机板烧录 工厂专用","错误 : "+strMsg)

def pop_up_Erase():
    messagebox.showinfo("Warning", "请确认ESP是否烧入完成")
...
def updateUI_Loader_finish():
    loader_btn.configure(text="开始烧录")
...
canvas_progress = ttk.Progressbar(window,orient=tkinter.HORIZONTAL,length=400,mode='indeterminate')
canvas_progress.pack(side=tk.TOP, fill='x', ipadx=0, padx=50, ipady=0, pady=20)



# 設定提示文字
lb = tk.Label(canvas_progress, text='烧录进度: ',font=fontStyle_14, bg=strBackgroudColor )
lb.pack(side=tk.LEFT, ipadx=0, padx=0, ipady=0, pady=0)

# 顯示版本號
lb23 = tk.Label(window, text='firmwareVersion:'+firmwareVersion,font=fontStyle_12,bg=strBackgroudColor, fg="#000000" )
lb23.pack(side=tk.BOTTOM, ipadx=0, padx=0, ipady=0, pady=0)
lb = tk.Label(window, text='ver:'+__version__,font=fontStyle_12,bg=strBackgroudColor, fg="#000000" )
lb.pack(side=tk.BOTTOM, ipadx=10, padx=0, ipady=0, pady=0)

# 燒錄按鈕
loader_btn = tk.Button(window, text='开始烧录',font=fontStyle_16, bg = "#008EDA", fg = "#FFFFFF", height = 3, width = 20, command=doStartThead_1)
...
def checkver():
    ...
    if exit_code:
        messagebox.showinfo("讯息","没有网路 无法确认是否有更新档")
    # 使用 GET 方式下載普通網頁
    else:
        ...
        if r.status_code == requests.codes.ok:
            ...
            print(t1, t2)
            if t2 > t1:
                messagebox.showinfo("讯息", "有新版本 按下确认后开始更新")
                r = requests.get('https://www.pingtex.tw/api/sys/firmwaredownloadurl?firmwareGuid='+str(fwguid)+'&firmwareVersion='+str(v2)+'&deviceGuid=TS10G-0000000&appOS=1&appVersion=4.0.1&countryCode=CN')
                print(r.status_code)
                if r.status_code == requests.codes.ok:
                    print("OK")
                    data = r.json()
                    ...
                    myDict = {
                        "Version": "1.0.0",
                        "firmwareVersion": str(v2)
                    }
                    print("UPdatajson")
                    with open("data.json", "w", encoding='UTF-8') as f:
                        json.dump(myDict, f, indent = 4)
                    lb23.configure(text='firmwareVersion:'+v2)
                    messagebox.showinfo("讯息", "更新完成")
        else:
            messagebox.showinfo("讯息", "连不上server")
checkver()
window.mainloop()
